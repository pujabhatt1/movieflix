...
public class MovieRepositoryImp implements MovieRepository {
	...
	public List<Movie> findMovieByText(String searchText) {
		...
		// em.createQuery("Select m from Movie m join m.movieCast where title like :pTitle OR actor like :pActor OR writer like :pWriter OR Director like :pDirector OR language like :pLanguage OR awards like :pAwards OR plot like :pPlot",Movie.class);
		TypedQuery<Movie> query = em.createNamedQuery("Movie.findMovieByText",
				Movie.class);
	...
	public List<Movie> sortMovieByField(String searchText, String field) {
		...
		if (field.equals("imdbRating")) {
			query = em.createNamedQuery("Movie.sortMoviesByImdbRating",
					Movie.class);
		} else if (field.equals("imdbVotes")) {
			query = em.createNamedQuery("Movie.sortMoviesByImdbVotes",
					Movie.class);
		} else if (field.equals("year")) {
			query = em.createNamedQuery("Movie.sortMoviesByImdbYear",
					Movie.class);
		} else {
	...
	public List<Movie> findMovieByTopRating(String type) {
		TypedQuery<Movie> query = em.createNamedQuery(
				"Movie.findTopRatedMovies", Movie.class);
	...
	public List<Movie> findMovieByImdbId(String imdbId) {
		TypedQuery<Movie> query = em.createNamedQuery(
				"Movie.findMovieByImdbId", Movie.class);
